...
function updateAuthUI() {
  ...
  if (currentUser) {
    ...
    providerBtns.forEach(b => { b.style.display = 'none'; });
    ...
    providerBtns.forEach(b => { b.style.display = 'inline-flex'; });
...
document.querySelectorAll('.auth-provider-btn').forEach(btn => {
  btn.addEventListener('click', () => signIn(btn.dataset.authProvider));
});
